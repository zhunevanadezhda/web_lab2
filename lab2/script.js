...
function initGame(game){
	...
	function func(cells,unic) {
			...
			for (var i = 0; i < cells.length; i++) {
				cells[i].addEventListener('click', function(){
					...
					if(thisColor == unic){
					...
					else{//если ошиблись, то таймер обновляется и игра начинается снова
						...
						//работает, но начинает быстрее считать время
						//нужно тут как-то очистить таймер
						clearInterval(idInt);
						doIt();
						newGame();
					}
	...
	function doIt() {
	        ...
	        idInt = setInterval(function() {
		            ...
		            //отсюда и ниже, сделать так, чтобы i остановилось на 0 и всё
		            if (i == 0) {
		                
		                alert("Время вышло!");
		                clearInterval(idInt);

		                //сделать так, чтобы нельзя было дальше играть
		                size=2;
						level = 1;
						amt = 200;
						dark = 20;
...
function restgame(size, field){
	...
	var all = Color();
	dark = dark+10;
	unic = UnicColor(all,amt-dark); //с каждым новым уровнем должно быть -бОльшее число
...
//превратим одномерный массив цветов в двумерный, чтобы потом по клеткам раскидать цвета
